...
void CMyCharacter::Init()
{
	...
	AABB[CHAR_BBSIZE][XDIM].Value = CS_XSIZE;
	AABB[CHAR_BBSIZE][YDIM].Value = CS_YSIZE;
	AABB[CHAR_BBSIZE][ZDIM].Value = CS_ZSIZE;
	...
	Radius = sqrt((AABB[CHAR_BBSIZE][XDIM].Value) * (AABB[CHAR_BBSIZE][XDIM].Value) + (AABB[CHAR_BBSIZE][YDIM].Value) * (AABB[CHAR_BBSIZE][YDIM].Value));// + (AABB[CHAR_BBSIZE][ZDIM].Value)*(AABB[CHAR_BBSIZE][ZDIM].Value));
	Radius *= 0.5f;
...
void CMyCharacter::AI_Move(void)
{
	float DeltaTime;
	Vector DeltaPos;
...
bool CMyCharacter::OutEvent(CS_SHIP_TRANSITION EventName) { return AI->outEvent(CS_NameTransition[EventName], NULL, this); }

///Physics methods
void CMyCharacter::Collided(CCharacter* CollidedChar)
{
	RTDESK_CMsg* Msg;
	...
	if (CHARS_COLLISION_TABLE[CHARS_SHIP][CollidedChar->Type])
		...
		case CHARS_SHIP:
			if (((CSIGame*)Directory[CHARS_GAME_REF])->BouncingMode)
			{
				CMyCharacter* other = (CMyCharacter*)CollidedChar;
...
void CMyCharacter::ChangeRenderMode(CHAR_RENDER_MODE Mode)
{
	RenderMode = Mode;
...
void CMyCharacter::Update(void)
{
	...
	if (Alive())					///If the Ship is alive
	{
		if (Timer[CS_UPD_PERIOD].IsSounding())
		{
			...
			Timer[CS_UPD_PERIOD].SetAlarm(Timer[CS_UPD_PERIOD].ms2Ticks(msUpdCMyCharacter));
...
void CMyCharacter::DiscreteUpdate(void)
{
	double msUpdShip;
...
void CMyCharacter::ReceiveMessage(RTDESK_CMsg* pMsg) {
...
void* init_ship(LPSTR* args, CMyCharacter* Ship)
{
	///This is the room for may be whatever additional actions to perform this current ship initialization
	return NULL;
}

void* CS_Move(LPSTR* args, CMyCharacter* Ship)
{
	Ship->AI_Move();
	return NULL;
}

void* CS_Display(LPSTR* args, CMyCharacter* Ship)
{
	Ship->Render();
	return NULL;
}

void* CS_Bursting(LPSTR* args, CMyCharacter* Ship)
{
	Ship->Explosion.Render();
	return NULL;
}

void* CS_Kill(LPSTR* args, CMyCharacter* Ship)
{
	Ship->EntombMe();
	return NULL;
}
